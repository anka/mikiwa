...
class ShoppingItems::ImageRecognizerTest < ActiveSupport::TestCase
  ...
  test "F80 enabled? false ohne API-Key in Credentials" do
    assert_not ShoppingItems::ImageRecognizer.enabled?
  end

  test "F80 enabled? true mit API-Key in Credentials" do
    with_stub(ShoppingItems::ImageRecognizer, :api_key, "sk-test") do
